...
#define EXPAND_VEC3F(v) v.x,v.y,v.z 
...
void Viewer::draw() {
  glBegin(GL_POINTS);
  for (auto it = solver_->refWorld()->refArrayMallas().begin(); it != solver_->refWorld()->refArrayMallas().end(); it++) {
	  for (auto V = (*it)->getVertexArray()->v_.begin(); V != (*it)->getVertexArray()->v_.end(); V++) {
		  glColor3f(1.0f, 0.2f, 0.2f);
		  glVertex3f(EXPAND_VEC3F((*V)->getPosition()));
	  }
  }
  glEnd();
  glBegin(GL_LINES);
  for (auto it = solver_->refWorld()->refArrayMallas().begin(); it != solver_->refWorld()->refArrayMallas().end(); it++) {
	  for (auto it2 = static_cast<mySim::MallaTriangular *> (*it)->getAristas().begin(); it2 != static_cast<mySim::MallaTriangular *> (*it)->getAristas().end(); it2++) {
		  glColor3f(0.2f, 0.0f, 0.9f);
		  glVertex3f(EXPAND_VEC3F((*it2)->v_[0]->getPosition()));
		  glVertex3f(EXPAND_VEC3F((*it2)->v_[1]->getPosition()));
		}
  }
  glEnd();
